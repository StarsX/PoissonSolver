...
HRESULT Jacobi::Init()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(h))
		{
			...
			if (SUCCEEDED(h)) m_uUAVSlot_b = desc.BindPoint;
			else hr = h;
...
void Jacobi::Solve(const XMUINT3 &vSize, ID3D11UnorderedAccessView *const pSrc, ID3D11UnorderedAccessView *const pDst, uint32_t uNumIt)
{
	const auto UAVInitialCounts = 0u;

	// Setup
	m_pd3dContext->CSSetUnorderedAccessViews(m_uUAVSlot_b, 1, &pSrc, &UAVInitialCounts);
	...
	m_pd3dContext->CSSetUnorderedAccessViews(m_uUAVSlot_b, 1, &g_pNullUAV, &UAVInitialCounts);
}
